...
def load_user(id):
	...
	if not login:
		return 
	return User(login['FName'], login['LName'], login['Email'], login['Password'], login['Country'], False)
...
def login():
	...
	if request.method == 'POST':
		...
		if not user:
			...
			return redirect(url_for('login'))
		if user.id != email or user.password != password:
			flash('Invalid username or password')
			return redirect(url_for('login'))
		login_user(user)
...
def create():
	...
	if request.method == 'POST':
		...
		user = User(fname, lname, email, password, country, False)
